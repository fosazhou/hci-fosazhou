...
export const otherWorks: OtherWork[] = [
  ...
  },
  {
    ...
    researchContent: {
      ...
      approach: "项目采用从数据提取到空间转译的设计路径。首先从声音频谱与波形曲线中提取频率、振��和��奏变化，再将其转化为参数化形态控制逻辑。随后通过曲面生成、结构细分和空间筛选，将抽象波形转译为屋顶系统，并进一步测试其作为公共界面、停留场所和场地标识的可能性。",
      logic: [
  ...
  },
  {
    ...
    fullDescriptionEn: "Using AIGC technology to explore digital design methods for Jiangnan water town architecture, won Second Prize at National College AIGC Digital Architecture and Cultural Product Design Competition.",
    year: "2024",
    startDate: "2024.9",
    endDate: "2024.11",
    category: "数智设计竞赛",
  ...
  },
  {
    ...
    researchContent: {
      problemStatement: "社区公共空间���环境品质往往受到建筑遮挡、日照不足、过度暴晒和活动空间分布不均的影响。传统设计过程中，光环境判断容易停留在经验层面，难以准确比较不同形态方案对日照、遮阳和使用舒适度的影响。项目关注的问题是：如何通��参数化工具和环境模拟，将光环境分析转化为可指导空间形态和公共活动组织的设计依据？",
