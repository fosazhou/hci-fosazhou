...
export const otherWorks: OtherWork[] = [
  {
    ...
    researchContentHk: {
      problemStatement: "在高密度既有城市片區中，醫療資源分佈、老齡社區需求與地面交通可達性之間存在錯位。傳統地面醫療物流依賴道路系統，容易受到交通擁堵、片區割裂和末端配送效率的限制。NESTIDE 關注的問題是：低空醫療物流能否通過建築節點嵌入既有城市結構，並轉化為一種可組��、可運營、可擴展的城市基礎設施？",
  ...
  },
  {
    ...
    researchContent: {
      problemStatement: "社区公共空间的环境品质往往受到建筑遮挡、日照不足、过度暴晒和活动空间分布不均的影响。传统设计过程中，光环境判断容易停留在经验层面，难以准确比较不同形态方案对日照、遮阳和使用舒适度的影响。项目关注的问题是：如何通过参数化工具和环境模拟，将光环境分析转化为可指导空间形态和公共活动组织的设计依据？",
      ...
      findings: "模拟结果显示，经过多轮参数化调整后，主要公共活动区域的有效日照表现较初始方案有所提升，其中关键区域的日照时长提升约 30%。这一结果说明，参数化环境分析能够有效辅助方案筛选，但仍需结合热舒适、遮阳需求和实际使用行为进行综合判断。",
